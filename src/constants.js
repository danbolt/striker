const GAME_WIDTH = 640;
const GAME_HEIGHT = 480;
const PLAYER_MOVE_CUTOFF = 8;
const MAP_WIDTH = 300;
const MAP_HEIGHT = 300;
const MAP_ORIGIN_X = (GAME_WIDTH - MAP_WIDTH) * 0.5;
const MAP_ORIGIN_Y = (GAME_HEIGHT - MAP_HEIGHT) * 0.5;
const ENTRY_INTO_BATTLE_DISTANCE = 50;
const ENTRY_INTO_BATTLE_DISTANCE_SQ = ENTRY_INTO_BATTLE_DISTANCE * ENTRY_INTO_BATTLE_DISTANCE;

const SHADER_WORLD_SCALE = 0.02;

const PLAYER_BULLET_SIZE = 8;
const PLAYER_BULLET_SPEED = 410;
const PLAYER_SHOT_DELAY_MS = 100;
const PLAYER_BULLET_POOL_SIZE = 48;
const PLAYER_STRIKE_TIME = 0.3877;
const PLAYER_SWORD_SEGMENTS = 10;
const PLAYER_SWORD_LENGTH = 96;

const EXPLOSION_POOL_SIZE = 12;

const ENEMY_POOL_SIZE = 64;
const ENEMY_BULLET_POOL_SIZE = 1024;

const DEFAULT_WORLD_SIZE = 1000;

// TODO: data driven for player
const PLAYER_MAX_HEALTH = 10;
const BULLET_DAMAGE = 3;
const REFLECT_DAMAGE = 11;
const PLAYER_DODGE_TIME_MS = 270;
const PLAYER_DODGE_RECHARGE_TIME_MS = 420;
const PLAYER_MOVE_SPEED = 230.47;
const PLAYER_DODGE_SPEED = 503.1;
const PLAYER_FLIGHT_SPEED = 7.8;

// TODO: data driven per enemy
const ENEMY_MAX_HEALTH = 22;
const ENEMY_COLLIDE_DAMAGE = 2;
const ENEMY_BULLET_DAMAGE = 1;
const ENEMY_BULLET_PERIOD_MS = 750;
const ENEMY_BULLET_SPEED = 300;
const ENEMY_MOVE_SPEED = 180;

const BULLET_NAME_KEY = 'bullet';
const ENEMY_NAME_KEY = 'enemy';


const FORMATIONS = {
  "SAMPLE_A": {
    //
  },
  "SAMPLE_B": {
    //
  },
};